...
//
// File: ApplyDifferentialCorrectionMa_j.cpp
//
...
#include "DDRTCM.h"
#include <cstring>
#include "ApplyDifferentialCorrectionMa_j.h"

// Function for MATLAB Function: '<S116>/Apply Differential Correction Mask1'
void DDRTCM::DDRTCM_do_vectors_g(const uint16_T a_data[], const int32_T *a_size,
...
void DDRTCM::DDRTCM_binary_expand_op_ekj(real_T in1_data[], int32_T *in1_size,
  const GNSS_Measurement *in2, const int32_T in4_data[], const int32_T *in4_size,
...
void DDRTCM::DDRTCM_binary_expand_op_ek(real_T in1_data[], int32_T *in1_size,
  const GNSS_Measurement *in2, const int32_T in4_data[], const int32_T *in4_size,
  const real_T in5[40], const int32_T in7_data[], const int32_T *in7_size)
{
  int32_T loop_ub;
...
//    '<S243>/Apply Differential Correction Mask1'
//
void DDRTCM::ApplyDifferentialCorrectionMa_j(const GNSS_Measurement
  ...
  *localDW)
{
  ...
  *rty_MeasurementCorrected = true;
  if ((*rtu_diffCorrAvailableSvidS1_DIM > 3) && rtu_enableDiffCorrection) {
    int32_T tmp;
    DDRTCM_do_vectors_g(rtu_diffCorrAvailableSvid,
                        rtu_diffCorrAvailableSvidS1_DIM,
                        rtu_GpsMeasurementBus->PRN,
                        &rtu_GpsMeasurementBus_DIMS1->PRN, localB->a__1_data,
                        &localB->a__1_size, localB->iia_data, &iia_size,
                        localB->iib_data, &iib_size);
    ...
    for (localB->i1 = 0; localB->i1 < localB->loop_ub; localB->i1++) {
      ...
      for (localB->i = 0; localB->i < localB->loop_ub_m; localB->i++) {
        localB->GPS_measurement_Bus_masked.satellite_position[localB->i + tmp *
          localB->i1] = rtu_GpsMeasurementBus->satellite_position
          [(rtu_GpsMeasurementBus_DIMS1_idx * localB->i1 + localB->
            iib_data[localB->i]) - 1];
      }
    }
    ...
    for (localB->i1 = 0; localB->i1 < localB->loop_ub; localB->i1++) {
      localB->loop_ub_m = iib_size;
      for (localB->i = 0; localB->i < localB->loop_ub_m; localB->i++) {
        localB->GPS_measurement_Bus_masked.satellite_velocity[localB->i + tmp *
          localB->i1] = rtu_GpsMeasurementBus->satellite_velocity
          [(rtu_GpsMeasurementBus_DIMS1_idx * localB->i1 + localB->
            iib_data[localB->i]) - 1];
      }
    }
    ...
    for (localB->i1 = 0; localB->i1 < localB->loop_ub; localB->i1++) {
      localB->GPS_measurement_Bus_masked.PRN[localB->i1] =
        rtu_GpsMeasurementBus->PRN[localB->iib_data[localB->i1] - 1];
    }
    ...
    if (iib_size == iia_size) {
      ...
      localB->loop_ub = iib_size;
      for (localB->i1 = 0; localB->i1 < localB->loop_ub; localB->i1++) {
        localB->tmp_data[localB->i1] = rtu_GpsMeasurementBus->pseudorange
          [localB->iib_data[localB->i1] - 1] -
          rtu_diffPseudorangeCorrections[localB->iia_data[localB->i1] - 1];
      }
    } else {
      DDRTCM_binary_expand_op_ekj(localB->tmp_data,
        &rtu_GpsMeasurementBus_DIMS1_idx, rtu_GpsMeasurementBus,
        localB->iib_data, &iib_size, rtu_diffPseudorangeCorrections,
        localB->iia_data, &iia_size);
    }
    ...
    if (localB->loop_ub - 1 >= 0) {
      std::memcpy(&localB->GPS_measurement_Bus_masked.pseudorange[0],
                  &localB->tmp_data[0], static_cast<uint32_T>(localB->loop_ub) *
                  sizeof(real_T));
    }
    ...
    localB->loop_ub = iib_size;
    for (localB->i1 = 0; localB->i1 < localB->loop_ub; localB->i1++) {
      localB->GPS_measurement_Bus_masked.pseudorange_raw[localB->i1] =
        rtu_GpsMeasurementBus->pseudorange_raw[localB->iib_data[localB->i1] - 1];
    }
    ...
    for (localB->i1 = 0; localB->i1 < localB->loop_ub; localB->i1++) {
      localB->GPS_measurement_Bus_masked.pseudorange_satclk_corrected[localB->i1]
        = rtu_GpsMeasurementBus->pseudorange_satclk_corrected[localB->
    ...
    localB->loop_ub = iib_size;
    for (localB->i1 = 0; localB->i1 < localB->loop_ub; localB->i1++) {
      localB->GPS_measurement_Bus_masked.deltarange[localB->i1] =
        rtu_GpsMeasurementBus->deltarange[localB->iib_data[localB->i1] - 1];
    }

    localDW->SFunction_DIMS2.deltarange_raw = iib_size;
    localB->loop_ub = iib_size;
    for (localB->i1 = 0; localB->i1 < localB->loop_ub; localB->i1++) {
      localB->GPS_measurement_Bus_masked.deltarange_raw[localB->i1] =
        rtu_GpsMeasurementBus->deltarange_raw[localB->iib_data[localB->i1] - 1];
    }
    ...
    for (localB->i1 = 0; localB->i1 < localB->loop_ub; localB->i1++) {
      localB->GPS_measurement_Bus_masked.deltarange_satclk_corrected[localB->i1]
        = rtu_GpsMeasurementBus->deltarange_satclk_corrected[localB->
        iib_data[localB->i1] - 1];
    }
    ...
    for (localB->i1 = 0; localB->i1 < localB->loop_ub; localB->i1++) {
      localB->GPS_measurement_Bus_masked.variance_pseudorange[localB->i1] =
        rtu_GpsMeasurementBus->variance_pseudorange[localB->iib_data[localB->i1]
        - 1];
    }
    ...
    localB->loop_ub = iib_size;
    for (localB->i1 = 0; localB->i1 < localB->loop_ub; localB->i1++) {
      localB->GPS_measurement_Bus_masked.variance_pseudorange_measured
        [localB->i1] = rtu_GpsMeasurementBus->
        variance_pseudorange_measured[localB->iib_data[localB->i1] - 1];
    }
    ...
    for (localB->i1 = 0; localB->i1 < localB->loop_ub; localB->i1++) {
      localB->GPS_measurement_Bus_masked.variance_deltarange[localB->i1] =
        rtu_GpsMeasurementBus->variance_deltarange[localB->iib_data[localB->i1]
        - 1];
    }
    ...
    if (iib_size == iia_size) {
      ...
      localB->loop_ub = iib_size;
      for (localB->i1 = 0; localB->i1 < localB->loop_ub; localB->i1++) {
        localB->tmp_data[localB->i1] = rtu_GpsMeasurementBus->
          carrierphase[localB->iib_data[localB->i1] - 1] -
          rtu_diffPseudorangeCorrections[localB->iia_data[localB->i1] - 1] /
          0.19029367279836487;
      }
    } else {
      DDRTCM_binary_expand_op_ek(localB->tmp_data,
        &rtu_GpsMeasurementBus_DIMS1_idx, rtu_GpsMeasurementBus,
        localB->iib_data, &iib_size, rtu_diffPseudorangeCorrections,
        localB->iia_data, &iia_size);
    }
    ...
    localB->loop_ub = iib_size;
    for (localB->i1 = 0; localB->i1 < localB->loop_ub; localB->i1++) {
      localB->GPS_measurement_Bus_masked.carrierphase_raw[localB->i1] =
        rtu_GpsMeasurementBus->carrierphase_raw[localB->iib_data[localB->i1] - 1];
    }

    localDW->SFunction_DIMS2.carrierphase_satclk_corrected = iib_size;
    localB->loop_ub = iib_size;
    for (localB->i1 = 0; localB->i1 < localB->loop_ub; localB->i1++) {
      localB->GPS_measurement_Bus_masked.carrierphase_satclk_corrected
        [localB->i1] = rtu_GpsMeasurementBus->
        carrierphase_satclk_corrected[localB->iib_data[localB->i1] - 1];
    }

    localDW->SFunction_DIMS2.variance_carrierphase_measured = iib_size;
    localB->loop_ub = iib_size;
    for (localB->i1 = 0; localB->i1 < localB->loop_ub; localB->i1++) {
      localB->GPS_measurement_Bus_masked.variance_carrierphase_measured
        [localB->i1] = rtu_GpsMeasurementBus->
        variance_carrierphase_measured[localB->iib_data[localB->i1] - 1];
    }

    localDW->SFunction_DIMS2.locktime = iib_size;
    localB->loop_ub = iib_size;
    for (localB->i1 = 0; localB->i1 < localB->loop_ub; localB->i1++) {
      localB->GPS_measurement_Bus_masked.locktime[localB->i1] =
        rtu_GpsMeasurementBus->locktime[localB->iib_data[localB->i1] - 1];
    }

    localDW->SFunction_DIMS2.CN0 = iib_size;
    localB->loop_ub = iib_size;
    for (localB->i1 = 0; localB->i1 < localB->loop_ub; localB->i1++) {
      localB->GPS_measurement_Bus_masked.CN0[localB->i1] =
        rtu_GpsMeasurementBus->CN0[localB->iib_data[localB->i1] - 1];
    }
