...
// Function for MATLAB Function: '<S56>/Calculate Dualfrequency Correction with L1 and L2C'
void DDRTCM::DDRTCM_do_vectors_l(const uint16_T a_data[], const int32_T *a_size,
  ...
  *ib_size, B_CalculateDualfrequencyCorre_T *localB)
{
  int32_T iafirst;
  int32_T ialast;
  int32_T ibfirst;
...
//    '<S183>/Calculate Dualfrequency Correction with L1 and L2C'
//
void DDRTCM::CalculateDualfrequencyCorrectio(const GNSS_Measurement
  *rtu_measurementBusL1, const GNSS_Measurement_size *rtu_measurementBusL1_DIMS1,
  const GNSS_Measurement *rtu_measurementBusL2C, const GNSS_Measurement_size
  *rtu_measurementBusL2C_DIMS1, boolean_T rtu_enableDualFrequencyCorrecti,
  B_CalculateDualfrequencyCorre_T *localB, DW_CalculateDualfrequencyCorr_T
  *localDW)
{
  ...
  localB->loop_ub = rtu_measurementBusL1_DIMS1->satellite_position[0] *
    rtu_measurementBusL1_DIMS1->satellite_position[1];
  ...
  localB->loop_ub = rtu_measurementBusL1_DIMS1->satellite_velocity[0] *
    rtu_measurementBusL1_DIMS1->satellite_velocity[1];
  if (localB->loop_ub - 1 >= 0) {
  ...
  if (rtu_enableDualFrequencyCorrecti) {
    ...
    DDRTCM_do_vectors_l(rtu_measurementBusL1->PRN,
                        &localB->rtu_measurementBusL1_DIMS1,
                        rtu_measurementBusL2C->PRN,
                        &localB->rtu_measurementBusL2C_DIMS1, localB->tmp_data,
                        &localB->tmp_size, localB->iia_data, &localB->iia_size,
                        localB->iib_data, &localB->iib_size, localB);
    localDW->SFunction_DIMS4 = localB->tmp_size;
    localB->loop_ub = localB->tmp_size;
    if (localB->loop_ub - 1 >= 0) {
      std::memcpy(&localB->SvidDualFrequencyCorrected[0], &localB->tmp_data[0],
    ...
    if (localDW->SFunction_DIMS4 != 0) {
      ...
      if (localB->i - 1 >= 0) {
        std::memset(&localB->dualFrequencyCorrection[0], 0, static_cast<uint32_T>
                    (localB->i) * sizeof(real_T));
      }

      localB->loop_ub = rtu_measurementBusL1_DIMS1->PRN;
      if (localB->loop_ub - 1 >= 0) {
        std::memset(&localB->idx2_data[0], 0, static_cast<uint32_T>
                    (localB->loop_ub) * sizeof(int8_T));
      }

      localB->loop_ub = rtu_measurementBusL1_DIMS1->PRN;
      if (localB->loop_ub - 1 >= 0) {
        std::memset(&localB->idx1_data[0], 0, static_cast<uint32_T>
                    (localB->loop_ub) * sizeof(int8_T));
      }
      ...
      for (localB->i = 0; localB->i < localB->c; localB->i++) {
        ...
        for (localB->vectorUB = 0; localB->vectorUB < localB->loop_ub;
             localB->vectorUB++) {
          if (rtu_measurementBusL1->PRN[localB->i] == rtu_measurementBusL2C->
              PRN[localB->vectorUB]) {
            localB->idx1_data[localB->i] = static_cast<int8_T>(localB->i + 1);
            localB->idx2_data[localB->i] = static_cast<int8_T>(localB->vectorUB
              + 1);
          }
        }
      }
      ...
      for (localB->c = 0; localB->c <= 148; localB->c += 2) {
        ...
        _mm_storeu_pd(&localB->fixed_f2[localB->c], _mm_div_pd(_mm_mul_pd
          (_mm_mul_pd(_mm_sub_pd(tmp, tmp_0), _mm_set1_pd(1.9836839845427221)),
           _mm_set1_pd(1.2276E+9)), _mm_set1_pd(1.57542E+9)));
      }
      ...
      for (localB->i = 0; localB->i < localB->vectorUB; localB->i++) {
        ...
        for (localB->c = 0; localB->c < localB->h_size_idx_0; localB->c++) {
          localB->g_data[localB->c] = (localB->fixed_PRN[localB->h_data
            [localB->c] - 1] == localB->rtu_measurementBusL1);
        }
        ...
        while ((!exitg1) && (localB->c <= localB->h_size_idx_0)) {
          if (localB->fixed_PRN[localB->k_data[localB->l_data[localB->c - 1] - 1]
              - 1] != 0) {
            y = true;
            exitg1 = true;
          } else {
        ...
        if (y) {
          localB->dualFrequencyCorrection[localB->i] = localB->
            fixed_f2[rtu_measurementBusL1->PRN[localB->i] - 1];
        }
      }
    } else {
      ...
      for (localB->c = 0; localB->c <= localB->vectorUB; localB->c += 2) {
        ...
        _mm_storeu_pd(&localB->dualFrequencyCorrection[localB->c], _mm_mul_pd
                      (_mm_set1_pd(0.0), tmp));
      }

      for (localB->c = localB->i; localB->c < localB->loop_ub; localB->c++) {
        localB->dualFrequencyCorrection[localB->c] = 0.0 *
          rtu_measurementBusL1->pseudorange[localB->c];
      }
    }
  } else {
    localDW->SFunction_DIMS3 = rtu_measurementBusL1_DIMS1->pseudorange;
