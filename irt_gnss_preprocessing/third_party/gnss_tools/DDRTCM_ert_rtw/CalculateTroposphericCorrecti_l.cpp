...
//
// File: CalculateTroposphericCorrecti_l.cpp
//
...
#include <cmath>
#include <emmintrin.h>
#include "CalculateTroposphericCorrecti_l.h"

// Function for MATLAB Function: '<S75>/Calculate Tropospheric Corrections'
real_T DDRTCM::DDRTCM_interp1_a(const real_T varargin_1[5], const real_T
  varargin_2[5], real_T varargin_3, B_CalculateTroposphericCorr_a_T *localB)
{
  real_T Vq;
  int32_T low_i;
  Vq = (rtNaN);
...
real_T DDRTCM::DDRTCM_rt_powd_snf_g(real_T u0, real_T u1,
...
// Function for MATLAB Function: '<S75>/Calculate Tropospheric Corrections'
void DDRTCM::DDRTCM_expand_power_g(const real_T a_data[], const int32_T *a_size,
  ...
  *c_size = csz_idx_0;
  if (csz_idx_0 != 0) {
    ...
    for (int32_T k{0}; k <= f; k++) {
      c_data[k] = DDRTCM_rt_powd_snf_g(a_data[d * k], b_data[e * k], localB);
    }
  }
}

void DDRTCM::DDRTCM_binary_expand_op_oz(real_T in1_data[], int32_T *in1_size,
  ...
  B_CalculateTroposphericCorr_a_T *localB)
{
  ...
  for (localB->i = 0; localB->i < loop_ub; localB->i++) {
    localB->varargout_1 = std::fmax(0.0, 4.0 - in10[localB->i * stride_10_0] /
      180.0 * 3.1415926535897931);
    localB->in2_tmp = in10[localB->i * stride_11_0];
    localB->in2_data[localB->i] = (109.65462799999999 / ((in5_data[localB->i *
      localB->stride_3_0] + 1.0) * 9.784 - in6_data[localB->i *
      localB->stride_4_0] * 287.054) * in7_data[localB->i * localB->stride_5_0] /
      in8_data[localB->i * localB->stride_6_0] * in4_data[localB->i *
      localB->stride_2_0] + in1_data[localB->i * localB->stride_1_0] *
      0.022276538615999996 / 9.784 * in2[localB->i * localB->stride_0_0]) *
      ((localB->varargout_1 * localB->varargout_1 * 0.015 + 1.0) * (1.001 /
        in12_data[localB->i * localB->stride_9_0]) * static_cast<real_T>
       ((localB->in2_tmp >= 0.034906585039886591) && (localB->in2_tmp <
         0.069813170079773182)) + 1.001 / in9_data[localB->i *
       localB->stride_7_0] * static_cast<real_T>(in10[localB->i *
        localB->stride_8_0] >= 0.087266462599716474));
  }

  *in1_size = in2_size_idx_0;
  if (in2_size_idx_0 - 1 >= 0) {
    std::memcpy(&in1_data[0], &localB->in2_data[0], static_cast<uint32_T>
                (in2_size_idx_0) * sizeof(real_T));
  }
}
...
//    '<S202>/Calculate Tropospheric Corrections'
//
void DDRTCM::CalculateTroposphericCorrecti_l(real_T rtu_dayOfYear, boolean_T
  rtu_enableTroposphericCorrectio, const real_T rtu_userPos_LLH[3], const real_T
  rtu_elevationAngle[40], const int32_T *rtu_elevationAngle_DIMS1,
  ...
  *localDW)
{
  ...
  if (rtu_enableTroposphericCorrectio) {
    ...
    if (localB->loop_ub - 1 >= 0) {
      std::memset(&localB->pressure_data[0], 0, static_cast<uint32_T>
    ...
    localB->loop_ub = static_cast<int8_T>(*rtu_elevationAngle_DIMS1);
    if (localB->loop_ub - 1 >= 0) {
      std::memset(&localB->temperature_data[0], 0, static_cast<uint32_T>
                  (localB->loop_ub) * sizeof(real_T));
    }

    localB->water_vapour_pressure_size = static_cast<int8_T>
      (*rtu_elevationAngle_DIMS1);
    localB->loop_ub = static_cast<int8_T>(*rtu_elevationAngle_DIMS1);
    if (localB->loop_ub - 1 >= 0) {
      std::memset(&localB->water_vapour_pressure_data[0], 0,
    ...
    localB->loop_ub = static_cast<int8_T>(*rtu_elevationAngle_DIMS1);
    if (localB->loop_ub - 1 >= 0) {
      std::memset(&localB->beta_data[0], 0, static_cast<uint32_T>
                  (localB->loop_ub) * sizeof(real_T));
    }
    ...
    localB->loop_ub = static_cast<int8_T>(*rtu_elevationAngle_DIMS1);
    if (localB->loop_ub - 1 >= 0) {
      std::memset(&localB->lambda_data[0], 0, static_cast<uint32_T>
                  (localB->loop_ub) * sizeof(real_T));
    }
    ...
    for (localB->k = 0; localB->k <= localB->vectorUB; localB->k += 2) {
      ...
      _mm_storeu_pd(&localB->y_data[localB->k], _mm_add_pd(_mm_mul_pd(tmp_1,
        tmp_1), _mm_set1_pd(0.002001)));
    }

    for (localB->k = localB->SFunction_DIMS2_tmp_tmp; localB->k <
         localB->loop_ub; localB->k++) {
    ...
    for (localB->k = 0; localB->k <= localB->vectorUB; localB->k += 2) {
      tmp_1 = _mm_loadu_pd(&localB->x_data[localB->k]);
      _mm_storeu_pd(&localB->x_data[localB->k], _mm_add_pd(_mm_mul_pd(tmp_1,
        tmp_1), _mm_set1_pd(0.002001)));
    }
    ...
    localB->latitude = rtu_userPos_LLH[0] * 180.0 / 3.1415926535897931;
    ...
    localB->cosinus_day = std::cos((rtu_dayOfYear - static_cast<real_T>
      (localB->k)) * 6.2831853071795862 / 365.25);
    for (localB->SFunction_DIMS2_tmp_tmp = 0; localB->SFunction_DIMS2_tmp_tmp < *
         rtu_elevationAngle_DIMS1; localB->SFunction_DIMS2_tmp_tmp++) {
      if (localB->latitude < 15.0) {
        localB->pressure_data[localB->SFunction_DIMS2_tmp_tmp] = 1013.25 - 0.0 *
          localB->cosinus_day;
        localB->temperature_data[localB->SFunction_DIMS2_tmp_tmp] = 299.65 - 0.0
          * localB->cosinus_day;
        localB->water_vapour_pressure_data[localB->SFunction_DIMS2_tmp_tmp] =
          26.31 - 0.0 * localB->cosinus_day;
        localB->beta_data[localB->SFunction_DIMS2_tmp_tmp] = 0.0063 - 0.0 *
          localB->cosinus_day;
        localB->lambda_data[localB->SFunction_DIMS2_tmp_tmp] = 2.77 - 0.0 *
          localB->cosinus_day;
      } else if (localB->latitude > 75.0) {
        localB->pressure_data[localB->SFunction_DIMS2_tmp_tmp] = 1013.0 - -0.5 *
          localB->cosinus_day;
        localB->temperature_data[localB->SFunction_DIMS2_tmp_tmp] = 263.65 -
          14.5 * localB->cosinus_day;
        localB->water_vapour_pressure_data[localB->SFunction_DIMS2_tmp_tmp] =
          4.11 - 3.39 * localB->cosinus_day;
        localB->beta_data[localB->SFunction_DIMS2_tmp_tmp] = 0.00453 - 0.00062 *
          localB->cosinus_day;
        localB->lambda_data[localB->SFunction_DIMS2_tmp_tmp] = 1.55 - 0.3 *
          localB->cosinus_day;
      } else {
        ...
        localB->pressure_data[localB->SFunction_DIMS2_tmp_tmp] =
          DDRTCM_interp1_a(localB->dv, j, localB->latitude, localB) -
          DDRTCM_interp1_a(localB->dv1, l, localB->latitude, localB) *
          localB->cosinus_day;
        for (localB->k = 0; localB->k < 5; localB->k++) {
          localB->d = 15.0 * static_cast<real_T>(localB->k) + 15.0;
          localB->dv[localB->k] = localB->d;
        ...
        localB->temperature_data[localB->SFunction_DIMS2_tmp_tmp] =
          DDRTCM_interp1_a(localB->dv, m, localB->latitude, localB) -
          DDRTCM_interp1_a(localB->dv1, n, localB->latitude, localB) *
          localB->cosinus_day;
        for (localB->k = 0; localB->k < 5; localB->k++) {
          localB->d = 15.0 * static_cast<real_T>(localB->k) + 15.0;
          localB->dv[localB->k] = localB->d;
        ...
        localB->water_vapour_pressure_data[localB->SFunction_DIMS2_tmp_tmp] =
          DDRTCM_interp1_a(localB->dv, o, localB->latitude, localB) -
          DDRTCM_interp1_a(localB->dv1, p, localB->latitude, localB) *
          localB->cosinus_day;
        for (localB->k = 0; localB->k < 5; localB->k++) {
          localB->d = 15.0 * static_cast<real_T>(localB->k) + 15.0;
          localB->dv[localB->k] = localB->d;
        ...
        localB->beta_data[localB->SFunction_DIMS2_tmp_tmp] = DDRTCM_interp1_a
          (localB->dv, q, localB->latitude, localB) - DDRTCM_interp1_a
          (localB->dv1, r, localB->latitude, localB) * localB->cosinus_day;
        for (localB->k = 0; localB->k < 5; localB->k++) {
          localB->d = 15.0 * static_cast<real_T>(localB->k) + 15.0;
          localB->dv[localB->k] = localB->d;
          localB->dv1[localB->k] = localB->d;
        ...
        localB->lambda_data[localB->SFunction_DIMS2_tmp_tmp] = DDRTCM_interp1_a
          (localB->dv, s, localB->latitude, localB) - DDRTCM_interp1_a
          (localB->dv1, t, localB->latitude, localB) * localB->cosinus_day;
      }
    }

    localB->latitude = rtu_userPos_LLH[2];
    localB->x_tmp_size = static_cast<int8_T>(*rtu_elevationAngle_DIMS1);
    localB->loop_ub = static_cast<int8_T>(*rtu_elevationAngle_DIMS1);
    localB->SFunction_DIMS2_tmp_tmp = (static_cast<int8_T>
      (*rtu_elevationAngle_DIMS1) / 2) << 1;
    localB->vectorUB = localB->SFunction_DIMS2_tmp_tmp - 2;
    for (localB->k = 0; localB->k <= localB->vectorUB; localB->k += 2) {
      tmp_1 = _mm_loadu_pd(&localB->beta_data[localB->k]);
      tmp_0 = _mm_loadu_pd(&localB->temperature_data[localB->k]);
      _mm_storeu_pd(&localB->x_tmp_data[localB->k], _mm_sub_pd(_mm_set1_pd(1.0),
        _mm_div_pd(_mm_mul_pd(tmp_1, _mm_set1_pd(localB->latitude)), tmp_0)));
    }

    for (localB->k = localB->SFunction_DIMS2_tmp_tmp; localB->k <
         localB->loop_ub; localB->k++) {
      localB->x_tmp_data[localB->k] = 1.0 - localB->beta_data[localB->k] *
        localB->latitude / localB->temperature_data[localB->k];
    }
    ...
    for (localB->k = 0; localB->k <= localB->vectorUB; localB->k += 2) {
      ...
      _mm_storeu_pd(&localB->b_b_data[localB->k], _mm_sub_pd(_mm_div_pd
        (_mm_div_pd(_mm_mul_pd(_mm_add_pd(tmp_1, tmp_0), _mm_set1_pd(9.80665)),
                    _mm_set1_pd(287.054)), tmp), tmp_0));
    }

    for (localB->k = localB->SFunction_DIMS2_tmp_tmp; localB->k <
         localB->loop_ub; localB->k++) {
      localB->b_b_data[localB->k] = (localB->lambda_data[localB->k] + 1.0) *
        9.80665 / 287.054 / localB->beta_data[localB->k] - 1.0;
    }

    if (localB->x_tmp_size == localB->b_b_size) {
      localB->loop_ub = localB->x_tmp_size;
      for (localB->k = 0; localB->k < localB->loop_ub; localB->k++) {
        localB->d_data[localB->k] = DDRTCM_rt_powd_snf_g(localB->
          x_tmp_data[localB->k], localB->b_b_data[localB->k], localB);
      }

      localDW->SFunction_DIMS2 = localB->x_tmp_size;
      localB->loop_ub = localB->x_tmp_size;
      if (localB->loop_ub - 1 >= 0) {
        std::memcpy(&localB->troposphericDelay[0], &localB->d_data[0],
                    static_cast<uint32_T>(localB->loop_ub) * sizeof(real_T));
      }
    } else {
      DDRTCM_expand_power_g(localB->x_tmp_data, &localB->x_tmp_size,
                            localB->b_b_data, &localB->b_b_size, localB->d_data,
                            &localB->d_size, localB);
    ...
    for (localB->k = 0; localB->k <= localB->vectorUB; localB->k += 2) {
      tmp_1 = _mm_loadu_pd(&localB->beta_data[localB->k]);
      _mm_storeu_pd(&localB->b_b_data[localB->k], _mm_div_pd(_mm_set1_pd
        (0.034163084297727957), tmp_1));
    }

    for (localB->k = localB->SFunction_DIMS2_tmp_tmp; localB->k <
         localB->loop_ub; localB->k++) {
      localB->b_b_data[localB->k] = 0.034163084297727957 / localB->
        beta_data[localB->k];
    }
    ...
    for (localB->k = 0; localB->k <= localB->vectorUB; localB->k += 2) {
      ...
      _mm_storeu_pd(&localB->dv9[0], _mm_sub_pd(_mm_set1_pd(4.0), _mm_mul_pd
        (_mm_div_pd(tmp_1, _mm_set1_pd(180.0)), _mm_set1_pd(3.1415926535897931))));
    ...
    for (localB->k = 0; localB->k <= localB->vectorUB; localB->k += 2) {
      tmp_1 = _mm_loadu_pd(&rtu_elevationAngle[localB->k]);
      _mm_storeu_pd(&localB->dv7[0], _mm_sub_pd(_mm_set1_pd(4.0), _mm_mul_pd
        (_mm_div_pd(tmp_1, _mm_set1_pd(180.0)), _mm_set1_pd(3.1415926535897931))));
    ...
    for (localB->k = 0; localB->k <= localB->vectorUB; localB->k += 2) {
      tmp_1 = _mm_loadu_pd(&rtu_elevationAngle[localB->k]);
      _mm_storeu_pd(&localB->dv5[0], _mm_sub_pd(_mm_set1_pd(4.0), _mm_mul_pd
        (_mm_div_pd(tmp_1, _mm_set1_pd(180.0)), _mm_set1_pd(3.1415926535897931))));
    ...
    for (localB->k = 0; localB->k <= localB->vectorUB; localB->k += 2) {
      tmp_1 = _mm_loadu_pd(&rtu_elevationAngle[localB->k]);
      _mm_storeu_pd(&localB->dv3[0], _mm_sub_pd(_mm_set1_pd(4.0), _mm_mul_pd
        (_mm_div_pd(tmp_1, _mm_set1_pd(180.0)), _mm_set1_pd(3.1415926535897931))));
    ...
    if ((localDW->SFunction_DIMS2 == static_cast<int8_T>
         ...
         (*rtu_elevationAngle_DIMS1) == localB->d_size) && ((localB->d_size == 1
          ? static_cast<int32_T>(static_cast<int8_T>(*rtu_elevationAngle_DIMS1))
          : localB->d_size) == (localDW->SFunction_DIMS2 == 1 ?
          static_cast<int32_T>(static_cast<int8_T>(*rtu_elevationAngle_DIMS1)) :
          localDW->SFunction_DIMS2)) && (((localDW->SFunction_DIMS2 == 1 ?
           static_cast<int32_T>(static_cast<int8_T>(*rtu_elevationAngle_DIMS1)) :
           localDW->SFunction_DIMS2) == 1 ? localB->d_size == 1 ?
          static_cast<int32_T>(static_cast<int8_T>(*rtu_elevationAngle_DIMS1)) :
          localB->d_size : localDW->SFunction_DIMS2 == 1 ? static_cast<int32_T>(
      ...
      for (localB->k = 0; localB->k < localDW->SFunction_DIMS2; localB->k++) {
        localB->latitude = std::fmax(0.0, 4.0 - rtu_elevationAngle[localB->k] /
          180.0 * 3.1415926535897931);
        localB->pressure_data[localB->k] = (109.65462799999999 /
          ((localB->lambda_data[localB->k] + 1.0) * 9.784 - localB->
           beta_data[localB->k] * 287.054) * localB->
          water_vapour_pressure_data[localB->k] / localB->
          temperature_data[localB->k] * localB->d_data[localB->k] +
          0.022276538615999996 * localB->pressure_data[localB->k] / 9.784 *
          localB->troposphericDelay[localB->k]) * ((localB->latitude *
          localB->latitude * 0.015 + 1.0) * (1.001 / localB->x_data[localB->k]) *
          static_cast<real_T>((rtu_elevationAngle[localB->k] >=
          0.034906585039886591) && (rtu_elevationAngle[localB->k] <
          0.069813170079773182)) + 1.001 / localB->y_data[localB->k] *
          static_cast<real_T>(rtu_elevationAngle[localB->k] >=
                              0.087266462599716474));
      }
    } else {
      DDRTCM_binary_expand_op_oz(localB->pressure_data, &localB->pressure_size,
        localB->troposphericDelay, localDW->SFunction_DIMS2, localB->d_data,
        &localB->d_size, localB->lambda_data, &localB->lambda_size,
        localB->beta_data, &localB->beta_size,
        localB->water_vapour_pressure_data, &localB->water_vapour_pressure_size,
        localB->temperature_data, &localB->temperature_size, localB->y_data,
        &localB->y_size, rtu_elevationAngle, rtu_elevationAngle_DIMS1,
        localB->x_data, &localB->x_size, localB);
    }

    localDW->SFunction_DIMS2 = localB->pressure_size;
    localB->loop_ub = localB->pressure_size;
    if (localB->loop_ub - 1 >= 0) {
      std::memcpy(&localB->troposphericDelay[0], &localB->pressure_data[0],
                  static_cast<uint32_T>(localB->loop_ub) * sizeof(real_T));
    }
  } else {
    localB->SFunction_DIMS2_tmp_tmp = static_cast<int8_T>
    ...
    if (localB->SFunction_DIMS2_tmp_tmp - 1 >= 0) {
      std::memset(&localB->troposphericDelay[0], 0, static_cast<uint32_T>
                  (localB->SFunction_DIMS2_tmp_tmp) * sizeof(real_T));
    }
  }
}
