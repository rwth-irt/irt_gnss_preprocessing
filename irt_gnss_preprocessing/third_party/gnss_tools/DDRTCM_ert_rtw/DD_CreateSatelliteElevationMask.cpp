...
//
// File: DD_CreateSatelliteElevationMask.cpp
//
...
#include "DD_CreateSatelliteElevationMask.h"

void DDRTCM::DDRTCM_binary_expand_op_c(real_T in1[3], const real_T in2[120],
...
void DDRTCM::DDRTCM_binary_expand_op_c3(boolean_T in1_data[], int32_T *in1_size,
...
real_T DDRTCM::DDRTCM_rt_atan2d_snf_b(real_T u0, real_T u1,
  B_CreateSatelliteElevationMas_T *localB)
{
  real_T y;
...
//    '<S186>/Create Satellite Elevation Mask'
//    '<S242>/Create Satellite Elevation Mask'
//
void DDRTCM::DD_CreateSatelliteElevationMask(const real_T rtu_userPosLlh[3],
  boolean_T rtu_userPos_isValid, const real_T rtu_satPosEcef[120], const int32_T
...
{
  localB->elevation_Angle_Limit = rtu_elevationMaskingAngle / 180.0 *
    3.1415926535897931;
  localDW->SFunction_DIMS3 = rtu_satPosEcef_DIMS1[0];
  localB->loop_ub = rtu_satPosEcef_DIMS1[0];
  if (localB->loop_ub - 1 >= 0) {
    std::memset(&localB->elevationAngle[0], 0, static_cast<uint32_T>
  ...
  localB->loop_ub = rtu_satPosEcef_DIMS1[0];
  if (localB->loop_ub - 1 >= 0) {
    std::memset(&localB->azimuthAngle[0], 0, static_cast<uint32_T>
                (localB->loop_ub) * sizeof(real_T));
  }
  ...
  localB->user_Position_ecef[0] = (localB->N + rtu_userPosLlh[2]) *
    localB->scale * localB->t;
  localB->u_unit_m = std::sin(rtu_userPosLlh[1]);
  localB->user_Position_ecef[1] = (localB->N + rtu_userPosLlh[2]) * std::cos
    (rtu_userPosLlh[0]) * localB->u_unit_m;
  localB->d = std::sin(rtu_userPosLlh[0]);
  localB->user_Position_ecef[2] = (0.99330562000985889 * localB->N +
    rtu_userPosLlh[2]) * localB->d;
  ...
  for (localB->b_k = 0; localB->b_k <= 0; localB->b_k += 2) {
    ...
    _mm_storeu_pd(&localB->e_unit[localB->b_k], _mm_add_pd(tmp_5, _mm_add_pd
      (tmp_4, _mm_add_pd(tmp_2, tmp))));
    tmp = _mm_add_pd(tmp_3, tmp_2);
    _mm_storeu_pd(&localB->n_unit[localB->b_k], _mm_add_pd(tmp_5, _mm_add_pd(tmp,
      tmp_0)));
    _mm_storeu_pd(&localB->u_unit[localB->b_k], _mm_add_pd(_mm_add_pd(tmp_4, tmp),
      tmp_1));
  }
  ...
  for (localB->nz = 0; localB->nz < localB->c; localB->nz++) {
    if (rtu_satPosEcef_DIMS1[1] == 3) {
      ...
      for (localB->b_k = 0; localB->b_k < localB->loop_ub; localB->b_k++) {
        localB->line_of_sight[localB->b_k] = rtu_satPosEcef
          [localB->rtu_satPosEcef_DIMS1_idx_0 * localB->b_k + localB->nz] -
          localB->user_Position_ecef[localB->b_k];
      }
    } else {
      DDRTCM_binary_expand_op_c(localB->line_of_sight, rtu_satPosEcef,
        rtu_satPosEcef_DIMS1, localB->nz, localB->user_Position_ecef, localB);
    }
    ...
    localB->elevationAngle[localB->nz] = std::asin((localB->line_of_sight[0] *
      localB->u_unit[0] + localB->line_of_sight[1] * localB->u_unit[1]) +
      localB->line_of_sight[2] * localB->u_unit[2]);
    localB->azimuthAngle[localB->nz] = DDRTCM_rt_atan2d_snf_b
  ...
  if (localDW->SFunction_DIMS3 == static_cast<int8_T>(localDW->SFunction_DIMS3))
  {
    ...
    {
      localB->tmp_data[localB->b_k] = (localB->elevationAngle[localB->b_k] >
        localB->elevation_Angle_Limit);
    }
  } else {
    DDRTCM_binary_expand_op_c3(localB->tmp_data, &localB->tmp_size,
      localB->elevationAngle, localDW->SFunction_DIMS3,
      localB->elevation_Angle_Limit, localB->b, localB);
  }

  localDW->SFunction_DIMS2 = localB->tmp_size;
  localB->loop_ub = localB->tmp_size;
  if (localB->loop_ub - 1 >= 0) {
    std::memcpy(&localB->elevationMask[0], &localB->tmp_data[0], static_cast<
                uint32_T>(localB->loop_ub) * sizeof(boolean_T));
  }
