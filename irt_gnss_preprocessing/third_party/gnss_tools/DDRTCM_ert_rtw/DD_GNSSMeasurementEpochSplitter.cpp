...
//
// File: DD_GNSSMeasurementEpochSplitter.cpp
//
...
#include "rtwtypes.h"
#include "DDRTCM.h"
#include "DD_GNSSMeasurementEpochSplitter.h"

// Function for MATLAB Function: '<S4>/GNSS Measurement Epoch Splitter'
int32_T DDRTCM::DDRTCM_combineVectorElements(const boolean_T x[40])
{
  int32_T y;
  y = x[0];
...
//
void DDRTCM::DD_GNSSMeasurementEpochSplitter(const gnssraw_measurement_t
  ...
  *rty_GalMeasurementBus, B_GNSSMeasurementEpochSplitte_T *localB)
{
  ...
  nz_tmp = DDRTCM_combineVectorElements(localB->v_m);
  i = DDRTCM_combineVectorElements(t);
  ...
  for (nz_tmp = 0; nz_tmp < x_size_idx_1; nz_tmp++) {
    rty_GpsMeasurementBus->SVID[nz_tmp] = rtu_MeasurementEpochBus->SVID
      [localB->x_data[nz_tmp] - 1];
  }
  ...
  for (nz_tmp = 0; nz_tmp < x_size_idx_1; nz_tmp++) {
    for (trueCount = 0; trueCount < 5; trueCount++) {
      rty_GpsMeasurementBus->Pseudorange[trueCount + 5 * nz_tmp] =
        rtu_MeasurementEpochBus->Pseudorange[(localB->y_data[nz_tmp] - 1) * 5 +
        trueCount];
    }
  }
  ...
  for (nz_tmp = 0; nz_tmp < x_size_idx_1; nz_tmp++) {
    for (trueCount = 0; trueCount < 5; trueCount++) {
      rty_GpsMeasurementBus->Pseudorange_Sigma[trueCount + 5 * nz_tmp] =
        rtu_MeasurementEpochBus->Pseudorange_Sigma[(localB->ab_data[nz_tmp] - 1)
        * 5 + trueCount];
    }
  }
  ...
  for (nz_tmp = 0; nz_tmp < x_size_idx_1; nz_tmp++) {
    for (trueCount = 0; trueCount < 5; trueCount++) {
      rty_GpsMeasurementBus->Doppler[trueCount + 5 * nz_tmp] =
        rtu_MeasurementEpochBus->Doppler[(localB->bb_data[nz_tmp] - 1) * 5 +
        trueCount];
    }
  }
  ...
  for (nz_tmp = 0; nz_tmp < x_size_idx_1; nz_tmp++) {
    for (trueCount = 0; trueCount < 5; trueCount++) {
      rty_GpsMeasurementBus->Type[trueCount + 5 * nz_tmp] =
        rtu_MeasurementEpochBus->Type[(localB->cb_data[nz_tmp] - 1) * 5 +
        trueCount];
    }
  }
  ...
  for (nz_tmp = 0; nz_tmp < x_size_idx_1; nz_tmp++) {
    for (trueCount = 0; trueCount < 5; trueCount++) {
      rty_GpsMeasurementBus->Carrier[trueCount + 5 * nz_tmp] =
        rtu_MeasurementEpochBus->Carrier[(localB->db_data[nz_tmp] - 1) * 5 +
        trueCount];
    }
  }
  ...
  for (nz_tmp = 0; nz_tmp < x_size_idx_1; nz_tmp++) {
    for (trueCount = 0; trueCount < 5; trueCount++) {
      rty_GpsMeasurementBus->Carrier_Sigma[trueCount + 5 * nz_tmp] =
        rtu_MeasurementEpochBus->Carrier_Sigma[(localB->eb_data[nz_tmp] - 1) * 5
        + trueCount];
    }
  }
  ...
  for (nz_tmp = 0; nz_tmp < x_size_idx_1; nz_tmp++) {
    for (trueCount = 0; trueCount < 5; trueCount++) {
      rty_GpsMeasurementBus->CN0[trueCount + 5 * nz_tmp] =
        rtu_MeasurementEpochBus->CN0[(localB->fb_data[nz_tmp] - 1) * 5 +
        trueCount];
    }
  }
  ...
  for (nz_tmp = 0; nz_tmp < x_size_idx_1; nz_tmp++) {
    for (trueCount = 0; trueCount < 5; trueCount++) {
      rty_GpsMeasurementBus->Locktime[trueCount + 5 * nz_tmp] =
        rtu_MeasurementEpochBus->Locktime[(localB->gb_data[nz_tmp] - 1) * 5 +
        trueCount];
    }
  }
  ...
  for (nz_tmp = 0; nz_tmp < trueCount; nz_tmp++) {
    rty_GalMeasurementBus->SVID[nz_tmp] = rtu_MeasurementEpochBus->SVID
      [localB->hb_data[nz_tmp] - 1];
  }
  ...
  for (nz_tmp = 0; nz_tmp < x_size_idx_1; nz_tmp++) {
    for (trueCount = 0; trueCount < 5; trueCount++) {
      rty_GalMeasurementBus->Pseudorange[trueCount + 5 * nz_tmp] =
        rtu_MeasurementEpochBus->Pseudorange[(localB->ib_data[nz_tmp] - 1) * 5 +
        trueCount];
    }
  }
  ...
  for (nz_tmp = 0; nz_tmp < x_size_idx_1; nz_tmp++) {
    for (trueCount = 0; trueCount < 5; trueCount++) {
      rty_GalMeasurementBus->Pseudorange_Sigma[trueCount + 5 * nz_tmp] =
        rtu_MeasurementEpochBus->Pseudorange_Sigma[(localB->jb_data[nz_tmp] - 1)
        * 5 + trueCount];
    }
  }
  ...
  for (nz_tmp = 0; nz_tmp < x_size_idx_1; nz_tmp++) {
    for (trueCount = 0; trueCount < 5; trueCount++) {
      rty_GalMeasurementBus->Doppler[trueCount + 5 * nz_tmp] =
        rtu_MeasurementEpochBus->Doppler[(localB->kb_data[nz_tmp] - 1) * 5 +
        trueCount];
    }
  }
  ...
  for (nz_tmp = 0; nz_tmp < x_size_idx_1; nz_tmp++) {
    for (trueCount = 0; trueCount < 5; trueCount++) {
      rty_GalMeasurementBus->Type[trueCount + 5 * nz_tmp] =
        rtu_MeasurementEpochBus->Type[(localB->lb_data[nz_tmp] - 1) * 5 +
        trueCount];
    }
  }
  ...
  for (nz_tmp = 0; nz_tmp < x_size_idx_1; nz_tmp++) {
    for (trueCount = 0; trueCount < 5; trueCount++) {
      rty_GalMeasurementBus->Carrier[trueCount + 5 * nz_tmp] =
        rtu_MeasurementEpochBus->Carrier[(localB->mb_data[nz_tmp] - 1) * 5 +
        trueCount];
    }
  }
  ...
  for (nz_tmp = 0; nz_tmp < x_size_idx_1; nz_tmp++) {
    for (trueCount = 0; trueCount < 5; trueCount++) {
      rty_GalMeasurementBus->Carrier_Sigma[trueCount + 5 * nz_tmp] =
        rtu_MeasurementEpochBus->Carrier_Sigma[(localB->nb_data[nz_tmp] - 1) * 5
        + trueCount];
    }
  }
  ...
  for (nz_tmp = 0; nz_tmp < x_size_idx_1; nz_tmp++) {
    for (trueCount = 0; trueCount < 5; trueCount++) {
      rty_GalMeasurementBus->CN0[trueCount + 5 * nz_tmp] =
        rtu_MeasurementEpochBus->CN0[(localB->ob_data[nz_tmp] - 1) * 5 +
        trueCount];
    }
  }
  ...
  for (nz_tmp = 0; nz_tmp < x_size_idx_1; nz_tmp++) {
    for (trueCount = 0; trueCount < 5; trueCount++) {
      rty_GalMeasurementBus->Locktime[trueCount + 5 * nz_tmp] =
        rtu_MeasurementEpochBus->Locktime[(localB->pb_data[nz_tmp] - 1) * 5 +
        trueCount];
    }
  }
