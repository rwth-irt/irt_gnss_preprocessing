  ...
  DW_SignalNoiseVariance_DDRTCM_T *localDW)
{
  if (rtu_use_measured_variance) {
    ...
    for (int32_T i{0}; i <= vectorUB; i += 2) {
      ...
      _mm_storeu_pd(&localB->varargin_2_tmp_data[i], _mm_div_pd(_mm_mul_pd(tmp,
    ...
    if (*rtu_carrierToNoiseRatio_DIMS1 - 1 >= 0) {
      std::memcpy(&localB->variancePseudorange[0], &localB->tmp_data[0],
    ...
    if (*rtu_carrierToNoiseRatio_DIMS1 - 1 >= 0) {
      std::memcpy(&localB->varianceDeltarange[0], &localB->tmp_data[0],
                  static_cast<uint32_T>(*rtu_carrierToNoiseRatio_DIMS1) * sizeof
                  (real_T));
    }
  }
