...
//
// File: GNS_NavigationDataConversionGAL.cpp
//
...
//    '<S18>/Navigation Data Conversion GAL'
//    '<S145>/Navigation Data Conversion GAL'
//
void GNSSPreProcessingDualAntenna::GNS_NavigationDataConversionGAL(const gnssraw_gal_nav_t *rtu_GAL_NAV_raw_Bus,
    B_NavigationDataConversionGAL_T *localB, DW_NavigationDataConversionGA_T *localDW)
{
    ...
    for (int32_T i{0}; i <= 34; i += 2)
    {
        ...
        _mm_storeu_pd(&localB->GAL_NavigationData_Bus.DELTA_N[i], _mm_mul_pd(tmp, tmp_0));
    ...
    for (int32_T i{0}; i <= 34; i += 2)
    {
        ...
        tmp_0 = _mm_set1_pd(3.1415926535898);
        _mm_storeu_pd(&localB->GAL_NavigationData_Bus.OMEGA_0[i], _mm_mul_pd(tmp, tmp_0));
        ...
        _mm_storeu_pd(&localB->GAL_NavigationData_Bus.omega[i], _mm_mul_pd(tmp, tmp_0));
        tmp = _mm_loadu_pd(&rtu_GAL_NAV_raw_Bus->OMEGADOT[i]);
        _mm_storeu_pd(&localB->GAL_NavigationData_Bus.OMEGADOT[i], _mm_mul_pd(tmp, tmp_0));
