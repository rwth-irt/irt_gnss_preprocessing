...
// Function for MATLAB Function: '<S111>/Apply Differential Correction Mask1'
void GNSS_preprocessingModelClass::GNSSPreProcessingS_do_vectors_i(const
...
void GNSS_preprocessingModelClass::GNSSPrePro_binary_expand_op_ok0(real_T
...
void GNSS_preprocessingModelClass::GNSSPreProc_binary_expand_op_ok(real_T
  in1_data[], int32_T *in1_size, const GNSS_Measurement *in2, const int32_T
  in4_data[], const int32_T *in4_size, const real_T in5[40], const int32_T
  in7_data[], const int32_T *in7_size)
{
...
// Output and update for atomic system: '<S111>/Apply Differential Correction Mask1'
void GNSS_preprocessingModelClass::ApplyDifferentialCorrectionMa_b(const
  ...
  *localDW)
{
  ...
  *rty_MeasurementCorrected = true;
  if ((*rtu_diffCorrAvailableSvidS1_DIM > 3) && rtu_enableDiffCorrection) {
    ...
    int32_T tmp;
    GNSSPreProcessingS_do_vectors_i(rtu_diffCorrAvailableSvid,
    ...
    if (iib_size == iia_size) {
      ...
      GNSSPrePro_binary_expand_op_ok0(tmp_data, &tmp_size, rtu_GpsMeasurementBus,
    ...
    if (iib_size == iia_size) {
      ...
      GNSSPreProc_binary_expand_op_ok(tmp_data, &tmp_size, rtu_GpsMeasurementBus,
        iib_data, &iib_size, rtu_diffPseudorangeCorrections, iia_data, &iia_size);
    }
