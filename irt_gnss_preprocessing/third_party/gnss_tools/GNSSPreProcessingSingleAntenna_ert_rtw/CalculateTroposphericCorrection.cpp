...
// Function for MATLAB Function: '<S23>/Calculate Tropospheric Corrections'
real_T GNSS_preprocessingModelClass::GNSSPreProcessingSingle_interp1(const
...
// Function for MATLAB Function: '<S23>/Calculate Tropospheric Corrections'
void GNSS_preprocessingModelClass::GNSSPreProcessingS_expand_power(const real_T
...
void GNSS_preprocessingModelClass::GNSSPreProc_binary_expand_op_n5(real_T
...
// Output and update for atomic system: '<S23>/Calculate Tropospheric Corrections'
void GNSS_preprocessingModelClass::CalculateTroposphericCorrection(real_T
  ...
  *rtu_elevationAngle_DIMS1, B_CalculateTroposphericCorrec_T *localB,
  DW_CalculateTroposphericCorre_T *localDW)
{
  ...
  if (rtu_enableTroposphericCorrectio) {
    ...
    latitude = rtu_userPosLlh[0] * 180.0 / 3.1415926535897931;
    if (latitude > 0.0) {
      k = 28;
    ...
    for (SFunction_DIMS2_tmp_tmp = 0; SFunction_DIMS2_tmp_tmp <
      ...
      if (latitude < 15.0) {
        ...
        pressure_data[SFunction_DIMS2_tmp_tmp] = GNSSPreProcessingSingle_interp1
          (tmp, j, latitude) - GNSSPreProcessingSingle_interp1(tmp_0, l,
        ...
        temperature_data[SFunction_DIMS2_tmp_tmp] =
          GNSSPreProcessingSingle_interp1(tmp, m, latitude) -
          GNSSPreProcessingSingle_interp1(tmp_0, n, latitude) * cosinus_day;
        for (k = 0; k < 5; k++) {
          tmp_c = 15.0 * static_cast<real_T>(k) + 15.0;
          tmp[k] = tmp_c;
          tmp_0[k] = tmp_c;
        ...
        water_vapour_pressure_data[SFunction_DIMS2_tmp_tmp] =
          GNSSPreProcessingSingle_interp1(tmp, o, latitude) -
          GNSSPreProcessingSingle_interp1(tmp_0, p, latitude) * cosinus_day;
        for (k = 0; k < 5; k++) {
          tmp_c = 15.0 * static_cast<real_T>(k) + 15.0;
          tmp[k] = tmp_c;
          tmp_0[k] = tmp_c;
        ...
        beta_data[SFunction_DIMS2_tmp_tmp] = GNSSPreProcessingSingle_interp1(tmp,
          q, latitude) - GNSSPreProcessingSingle_interp1(tmp_0, r, latitude) *
          cosinus_day;
        for (k = 0; k < 5; k++) {
          tmp_c = 15.0 * static_cast<real_T>(k) + 15.0;
          tmp[k] = tmp_c;
        ...
        lambda_data[SFunction_DIMS2_tmp_tmp] = GNSSPreProcessingSingle_interp1
          (tmp, s, latitude) - GNSSPreProcessingSingle_interp1(tmp_0, t,
          latitude) * cosinus_day;
      }
    }

    latitude = rtu_userPosLlh[2];
    ...
    if (static_cast<int8_T>(*rtu_elevationAngle_DIMS1) == static_cast<int8_T>
      ...
      GNSSPreProcessingS_expand_power(x_tmp_data, &x_tmp_size, b_b_data,
    ...
    if ((localDW->SFunction_DIMS2 == static_cast<int8_T>
      ...
      GNSSPreProc_binary_expand_op_n5(pressure_data, &pressure_size,
