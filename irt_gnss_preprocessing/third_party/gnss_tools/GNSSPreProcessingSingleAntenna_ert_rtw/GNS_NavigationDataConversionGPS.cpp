...
// Output and update for atomic system: '<S11>/Navigation Data Conversion GPS'
void GNSS_preprocessingModelClass::GNS_NavigationDataConversionGPS(const
  gnssraw_gps_nav_t *rtu_GpsNavBus, B_NavigationDataConversionGPS_T *localB,
  DW_NavigationDataConversionGP_T *localDW)
{
  ...
  for (int32_T i{0}; i <= 34; i += 2) {
    ...
    _mm_storeu_pd(&localB->GpsNavBusReshaped.DELTA_N[i], _mm_mul_pd(tmp,
  ...
  for (int32_T i{36}; i < 37; i++) {
    ...
    localB->GpsNavBusReshaped.DELTA_N[i] = rtu_GpsNavBus->DELTA_N[i] *
      3.1415926535897931;
  }
  ...
  for (int32_T i{0}; i <= 34; i += 2) {
    ...
    _mm_storeu_pd(&localB->GpsNavBusReshaped.M_0[i], _mm_mul_pd(tmp, _mm_set1_pd
      (3.1415926535897931)));
  }

  for (int32_T i{36}; i < 37; i++) {
    localB->GpsNavBusReshaped.M_0[i] = rtu_GpsNavBus->M_0[i] *
      3.1415926535897931;
  }
  ...
  for (int32_T i{0}; i <= 34; i += 2) {
    ...
    _mm_storeu_pd(&localB->GpsNavBusReshaped.OMEGA_0[i], _mm_mul_pd(tmp,
      _mm_set1_pd(3.1415926535897931)));
  }

  for (int32_T i{36}; i < 37; i++) {
    localB->GpsNavBusReshaped.C_ic[i] = rtu_GpsNavBus->C_ic[i];
    localB->GpsNavBusReshaped.OMEGA_0[i] = rtu_GpsNavBus->OMEGA_0[i] *
      3.1415926535897931;
  }
  ...
  for (int32_T i{0}; i <= 34; i += 2) {
    ...
    _mm_storeu_pd(&localB->GpsNavBusReshaped.I_0[i], _mm_mul_pd(tmp, _mm_set1_pd
      (3.1415926535897931)));
  }

  for (int32_T i{36}; i < 37; i++) {
    localB->GpsNavBusReshaped.C_is[i] = rtu_GpsNavBus->C_is[i];
    localB->GpsNavBusReshaped.I_0[i] = rtu_GpsNavBus->I_0[i] *
      3.1415926535897931;
  }
  ...
  for (int32_T i{0}; i <= 34; i += 2) {
    ...
    _mm_storeu_pd(&localB->GpsNavBusReshaped.omega[i], _mm_mul_pd(tmp,
      _mm_set1_pd(3.1415926535897931)));
  }

  for (int32_T i{36}; i < 37; i++) {
    localB->GpsNavBusReshaped.C_rc[i] = rtu_GpsNavBus->C_rc[i];
    localB->GpsNavBusReshaped.omega[i] = rtu_GpsNavBus->omega[i] *
      3.1415926535897931;
  }
  ...
  for (int32_T i{0}; i <= 34; i += 2) {
    ...
    _mm_storeu_pd(&localB->GpsNavBusReshaped.OMEGADOT[i], _mm_mul_pd(tmp,
      _mm_set1_pd(3.1415926535897931)));
  }

  for (int32_T i{36}; i < 37; i++) {
    localB->GpsNavBusReshaped.OMEGADOT[i] = rtu_GpsNavBus->OMEGADOT[i] *
      3.1415926535897931;
  }
  ...
  for (int32_T i{0}; i <= 34; i += 2) {
    ...
    _mm_storeu_pd(&localB->GpsNavBusReshaped.IDOT[i], _mm_mul_pd(tmp,
      _mm_set1_pd(3.1415926535897931)));
  }

  for (int32_T i{36}; i < 37; i++) {
    localB->GpsNavBusReshaped.IDOT[i] = rtu_GpsNavBus->IDOT[i] *
      3.1415926535897931;
  }
}
